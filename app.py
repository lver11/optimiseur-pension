...
try:
    ...
    with st.sidebar:
        ...
        vol_target = st.slider("Cible Volatilité Max (%)", 5.0, 20.0, 10.0) / 100
    ...
    with tab_lex:
        ...
        with c1:
            st.markdown("""
            ### Gestion du Risque
            * **Alpha (Délissage) :** Ajustement de la volatilité pour les actifs évalués périodiquement.
            * **Placement Privé Fixe :** Ancre mathématique pour stabiliser l'optimisation.
            """)
        with c2:
            st.markdown("""
            ### Ingénierie Financière
            * **TER (Total Expense Ratio) :** Frais totaux pondérés incluant le coût du levier.
            """)
    ...
    with tab_opt:
        ...
        if w_opt is not None:
            ...
            for i, t in enumerate(targets):
                w_s = optimize_portfolio(exp_raw - applied_fees_series, cov_base, lev_max, t, borrow_cost, asset_bounds, 0.5, pe_fix, vol_target)
                if w_s is not None:
                    v_s = np.sqrt(w_s.T @ cov_base @ w_s)
                    r_s = (w_s @ (exp_raw - applied_fees_series)) - ((np.sum(w_s)-1) * borrow_cost if np.sum(w_s) > 1 else 0)
                    res = {"Profil": f"P{i+1}", "Rendement": r_s, "Volatilité": v_s}
                    for j, asset_name in enumerate(TICKERS_DICT.keys()):
                        res[asset_name] = w_s[j]
                    frontier_data.append(res)
            
            if frontier_data:
                df_f = pd.DataFrame(frontier_data)
                fig_mig = px.area(df_f.melt(id_vars=["Profil", "Rendement", "Volatilité"], var_name="Actif", value_name="Poids"), 
                                  x="Profil", y="Poids", color="Actif", title="Migration de l'Allocation")
