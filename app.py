...
try:
    ...
    h1, h2 = st.columns(2)
    with h1:
        with st.expander("🔍 Hypothèses de Marché"):
            stats = pd.DataFrame({
    ...
    with h2:
        with st.expander("📈 Corrélations"):
            st.plotly_chart(px.imshow(hist_rets.corr(), text_auto=".2f", color_continuous_scale='RdBu_r'), use_container_width=True)
    ...
    if w_opt is not None:
        ...
        st.divider()
        st.subheader("🏁 Comparaison vs 60/40")
